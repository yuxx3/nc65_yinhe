...
/**
 * @author yuxx3
 * @created at 2016年6月2日,下午1:58:05
 *
 */
public class GdorderFCImpl implements IGdOrderNM {
	...
	public GDOrderCompStatus[] queryByDataVisibilitySetting(
		...
		//执行存储过程
		String sql = "call pkg_report_pro.gd_ordercompstatus("+AppContext.getInstance().getPkUser()+",'d')";
		execute(sql);
		// (不走管控模式的查询)
		//贸易类型（def1）为FC-d
		StringBuffer whereCondition = new StringBuffer("def1 = 'd'");
		if (paramString != null) {
