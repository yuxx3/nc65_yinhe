 ...
 * 
 * @since 6.0
 * @version 2010-11-26 上午10:50:11
 * @author yinyxa
 */
public class DelivNumUpdateOperator implements ICompareRule<DelivBillAggVO> {
	/**
	 * MAP集合，用来保存单据修改前需要的字段信息,key:来源单据类型+表体id，value：单据修改前数量
	 */
	// private Map<String, UFDouble> map;

	// 数据库中已经保存的修改前的信息
	// private DelivBillAggVO[] riginBills;

	// public DelivNumUpdateOperator() {
	// super();
	// this.riginBills = bills;
	// }

	// public DelivBillAggVO[] getRiginBills() {
	// return this.riginBills;
	// }

	@Override
	public void process(DelivBillAggVO[] vos, DelivBillAggVO[] originVOs) {
		...
		// mapping.setSrcTSKey(DelivBillBVO.TS);
		...
		// mapping.setSrcbTSKey(BillItemVO.SRCBTS);
		// mapping.setCsrcbbidKey(DelivBillBVO.CPUORDER_BB1ID);
		...
		Map<String, List<RewritePara>> paraIndex = tool.splitForUpdate(vos,
				originVOs);
